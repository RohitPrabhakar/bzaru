...
class CInvite extends StatefulWidget {
  @override
  _CInviteState createState() => _CInviteState();
}

class _CInviteState extends State<CInvite> {
  ...
  void shareToWhatsApp() async {
    ...
    try {
      await SocialShare.shareWhatsapp(url).then(
        (value) => value != "true"
            ? Utility.displaySnackbar(context,
                msg: value?.capitalize(), key: _scaffoldKey)
            : null,
      );
    } catch (e) {
      Utility.displaySnackbar(context, msg: e.toString(), key: _scaffoldKey);
    ...
    try {
      await SocialShare.shareFacebookStory("", "#ffffff", "#000000", url,
          ...
          .then((value) => value != "true"
              ...
              : null)
          .onError(
            (error, stackTrace) async => isAndroid
                ? await FlutterShareMe().shareToFacebook(msg: url)
                : null,
          );
    } catch (e) {
      Utility.displaySnackbar(context, msg: e.toString(), key: _scaffoldKey);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      key: _scaffoldKey,
      appBar: BAppBar(
        title: locale.getTranslatedValue(KeyConstants.referrals),
          ...
          children: [
            Column(
              children: [
                ...
                SizedBox(height: 30),
                BText(
                  locale.getTranslatedValue(KeyConstants.inviteContacts),
                ...
                SizedBox(height: 30),
                BText(
                  locale.getTranslatedValue(KeyConstants.sendInvites),
                ...
                SizedBox(height: 5),
                BText(
                  locale.getTranslatedValue(KeyConstants.contactsWillBeSaved),
                  variant: TypographyVariant.h2,
                  textAlign: TextAlign.center,
                  style: TextStyle(
                    letterSpacing: 0.5,
            ...
            Row(
              ...
              children: [
                ...
                _buildShareRow(
                    ...
                    locale.getTranslatedValue(KeyConstants.share),
                    shareToFacebook),
                _buildShareRow(KImages.moreIcon,
                    locale.getTranslatedValue(KeyConstants.share), share),
              ],
            )
          ],
        ),
